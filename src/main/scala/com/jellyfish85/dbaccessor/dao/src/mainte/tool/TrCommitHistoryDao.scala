...
class TrCommitHistoryDao extends GeneralDao[TrCommitHistoryBean] {
   ...
   */
  @throws(classOf[SQLException])
  def findDiffs(conn: Connection, leftBaseUrl: String): List[TrCommitHistoryBean] = {
    var list: List[TrCommitHistoryBean] = List()

    val sql:  String = generateSimpleQuery("/query/src/mainte/tool/SELECT_TR_COMMIT_HISTORY_DIFFS.sql")
    val stmt: PreparedStatement = conn.prepareStatement(sql)
   ...
   */
  @throws(classOf[SQLException])
  def findNotExists(conn: Connection, leftBaseUrl: String): List[TrCommitHistoryBean] = {
    var list: List[TrCommitHistoryBean] = List()

    val sql:  String = generateSimpleQuery("/query/src/mainte/tool/SELECT_TR_COMMIT_HISTORY_NOT_EXISTS.sql")
    val stmt: PreparedStatement = conn.prepareStatement(sql)
